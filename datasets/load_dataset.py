...
def iris(shuffle=False) -> tuple[pd.DataFrame, pd.DataFrame]:
  ...
  data = pd.read_csv(current + "/iris_dataset/iris.data", header=None)
...
def rice(shuffle=False) -> tuple[pd.DataFrame, pd.DataFrame]:
  current = os.path.dirname(os.path.abspath(__file__))
  data = pd.read_csv(current + "/rice_dataset/Rice_Cammeo_Osmancik.csv", header=None)
  if (shuffle):
    data = data.sample(frac=1).reset_index(drop=True)
...
def wine():
  current = os.path.dirname(os.path.abspath(__file__))
  data = pd.read_csv(current + "/wine_dataset/wine.csv", header=None)
